...
describe("action_executed payload schema", () => {
  ...
  it("accepts BREAK_EVEN with optional detail", () => {
    ...
    expect(
      schema.safeParse({
        ...base,
        action: "BREAK_EVEN",
        detail: { appliedAt: "2026-07-12T00:00:00Z" },
      }).success,
    ).toBe(true);
  });
  ...
  it("REQUIRES newStopPrice on TRAILING_MOVE", () => {
    expect(
      schema.safeParse({ ...base, action: "TRAILING_MOVE", detail: { newStopPrice: 1.2345 } })
        .success,
    ).toBe(true);
  ...
  it("REQUIRES levelId + executedPrice + closedVolume on PARTIAL_TP", () => {
    expect(
      schema.safeParse({
        ...base,
        action: "PARTIAL_TP",
        detail: { levelId: "tp-1", executedPrice: 1, closedVolume: 0.5 },
      }).success,
    ).toBe(true);
    expect(
      schema.safeParse({
        ...base,
        action: "PARTIAL_TP",
        detail: { levelId: "tp-1", executedPrice: 1 },
      }).success,
    ).toBe(false);
    expect(
      schema.safeParse({
        ...base,
        action: "PARTIAL_TP",
        detail: { levelId: "tp-1", closedVolume: 0.5 },
      }).success,
    ).toBe(false);
  });
...
describe("action_failed payload schema", () => {
  ...
  it("rejects a failure missing reason or retryable", () => {
    expect(schema.safeParse({ ...base, action: "TRAILING_MOVE", retryable: true }).success).toBe(
      false,
    );
