...
describe("management projection lifecycle", () => {
  it("plan_changed installs the plan", () => {
    const next = applyManagementEvent(
      undefined,
      env("position.management.plan_changed", { positionId, plan: seedPlan }, 1),
    );
  ...
  it("BE action_executed flips breakEven.state to APPLIED", () => {
    let s = applyManagementEvent(
      undefined,
      env("position.management.plan_changed", { positionId, plan: seedPlan }, 1),
    )!;
    s = applyManagementEvent(
      s,
      env(
        "position.management.action_executed",
        { positionId, planId, action: "BREAK_EVEN", detail: {} },
        2,
      ),
    )!;
  ...
  it("two TRAILING_MOVE events increment moveCount and update stop", () => {
    let s: PositionManagementProjection | undefined = applyManagementEvent(
      undefined,
      env("position.management.plan_changed", { positionId, plan: seedPlan }, 1),
    );
    s = applyManagementEvent(
      ...
      env(
        "position.management.action_executed",
        { positionId, planId, action: "TRAILING_MOVE", detail: { newStopPrice: 1.0862 } },
        2,
      ),
    );
    s = applyManagementEvent(
      ...
      env(
        "position.management.action_executed",
        { positionId, planId, action: "TRAILING_MOVE", detail: { newStopPrice: 1.0874 } },
        3,
      ),
    );
    ...
    s = applyManagementEvent(
      ...
      env(
        "position.management.action_executed",
        {
          ...
          planId,
          action: "PARTIAL_TP",
          detail: { levelId: "tp-1", executedPrice: 1.088, closedVolume: 0.5 },
        },
        2,
      ),
    );
  ...
  it("ignores non-management envelopes", () => {
    const start = applyManagementEvent(
      undefined,
      env("position.management.plan_changed", { positionId, plan: seedPlan }, 1),
    )!;
