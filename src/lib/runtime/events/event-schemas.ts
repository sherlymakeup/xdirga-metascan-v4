...
export const handshakeSchema = z
  .object({
    ...
    capabilitiesRevision: nonNegativeInt,
    minFrontendVersion: z.string().optional(),
    frontendVersion: z.string().optional(),
    supportedFeatures: z.array(z.string()),
    supportedCommands: z.array(z.string()),
    brokerProvider: z.literal("EXNESS"),
    brokerEnvironment: z.enum(["TRIAL", "LIVE"]),
    executionSemantics: z.literal("LIVE"),
...
const management = z.object({
  ...
  breakEven: z.object({
    ...
    appliedAt: timeOrNull,
  }),
  trailing: z.object({
    mode: z.enum(["OFF", "FIXED_POINTS", "ATR", "STEP", "STRUCTURE"]),
    active: z.boolean(),
    distancePoints: numberOrNull,
    currentStopPrice: numberOrNull,
    lastMovedAt: timeOrNull,
    moveCount: nonNegativeInt,
  }),
  partialTp: z.object({
    levels: z.array(
      z.object({
        ...
        executedAt: timeOrNull,
        executedPrice: numberOrNull,
        closedVolume: numberOrNull,
      }),
    ),
  }),
  timeExit: z.object({
    maxHoldUntil: timeOrNull,
...
const cockpitSnapshotSchema = z.object({
  positionsAvailable: z.boolean(),
  positionsSourceFrameId: numberOrNull,
  positionsObservedAt: timeOrNull,
  accountAvailable: z.boolean(),
  accountSourceFrameId: numberOrNull,
  accountObservedAt: timeOrNull,
  runtime: z.object({
    id: text,
    sessionId: text,
    version: text,
    ...
    tradingMode: z.enum(["TRIAL", "LIVE"]),
    state: runtimeState,
    previousState: runtimeState,
    stateChangedAt: timeOrNull,
    stateReason: text,
    startedAt: timeOrNull,
    uptimeSec: numberOrNull,
    lastHeartbeatAt: timeOrNull,
  ...
  subsystems: z.array(
    z.object({
      ...
      latencyMs: numberOrNull,
      restartCount: z.number(),
      currentAction: text.nullable(),
      lastError: text.nullable(),
    }),
  ),
  broker: z.object({
    ...
    server: text,
    loginMasked: text,
    accountMode: z.enum(["TRIAL", "LIVE"]),
    connection: z.enum(["CONNECTED", "DISCONNECTED", "RECONNECTING", "DEGRADED", "UNKNOWN"]),
    tradingPermitted: z.boolean(),
    terminalVersion: text,
    lastTickAt: timeOrNull,
    lastRequestAt: timeOrNull,
    queueDepth: z.number(),
    avgLatencyMs: numberOrNull,
  ...
  account: z.object({
    ...
    equity: numberOrNull,
    margin: numberOrNull,
    freeMargin: numberOrNull,
    marginLevel: numberOrNull,
    floatingPnl: numberOrNull,
    realizedPnlToday: numberOrNull,
    realizedPnlWeek: numberOrNull,
    dailyDrawdown: numberOrNull,
    maxDrawdown: numberOrNull,
    grossExposure: numberOrNull,
    netExposure: numberOrNull,
    openPositions: numberOrNull,
    pendingOrders: numberOrNull,
    tradesToday: numberOrNull,
    winRate: numberOrNull,
    profitFactor: numberOrNull,
    riskUtilization: numberOrNull,
    updatedAt: timeOrNull,
    freshness,
  }),
  strategies: z.array(
    z.object({
      ...
      lastSignalAt: timeOrNull,
      lastExecutionAt: timeOrNull,
      currentBias: z.enum(["LONG", "SHORT", "FLAT"]),
      confidence: z.number(),
  ...
  positions: z.array(
    z.object({
      id: text,
      brokerTicket: text,
      ownership: z.enum(["BOT_MANAGED", "FOREIGN", "UNKNOWN"]),
      dataAvailable: z.boolean(),
      sourceFrameId: z.number(),
      observedAt: timeOrNull,
      symbol: text,
      side: orderSide,
      volume: z.number(),
      entryPrice: z.number(),
      currentPrice: z.number(),
      stopLoss: numberOrNull,
      takeProfit: numberOrNull,
      floatingPnl: z.number(),
      realizedPnl: numberOrNull,
      riskAmount: numberOrNull,
      riskPct: numberOrNull,
      openedAt: timeOrNull,
      ...
      state: z.enum(["OPEN", "CLOSING", "CLOSED", "RECONCILE_REQUIRED"]),
      rMultiple: numberOrNull,
      mfe: numberOrNull,
      mae: numberOrNull,
      commission: z.number(),
      swap: z.number(),
  ...
  orders: z.array(
    z.object({
      ...
      volume: z.number(),
      requestedPrice: numberOrNull,
      filledPrice: numberOrNull,
      stopLoss: numberOrNull,
      takeProfit: numberOrNull,
      slippage: numberOrNull,
      strategy: text,
      createdAt: timestamp,
      updatedAt: timestamp,
      status: z.enum([
  ...
  markets: z.array(
    z.object({
      symbol: text,
      group: z.enum(["FX", "METALS", "INDICES", "CRYPTO"]).nullable(),
      bid: z.number(),
      ask: z.number(),
      spread: z.number(),
      last: z.number(),
      changePct: numberOrNull,
      sessionOpen: z.boolean().nullable(),
      tradingPermitted: z.boolean(),
      tickAgeMs: z.number(),
      freshness,
      ...
      maxVolume: z.number(),
      volumeStep: z.number(),
      swapLong: numberOrNull,
      swapShort: numberOrNull,
      marginRequirement: numberOrNull,
    }),
  ),
  alerts: z.array(
    z.object({
  ...
  incidents: z.array(
    z.object({
      id: text,
      severity,
      status: z.enum(["OPEN", "ACKNOWLEDGED", "INVESTIGATING", "MITIGATED", "RESOLVED"]),
      title: text,
      startedAt: timestamp,
      durationSec: z.number(),
      source: text,
      impact: text,
      affectedComponents: z.array(text),
  ...
  riskLimits: z.array(
    z.object({
      ...
      configured: z.number(),
      current: z.number(),
      unit: z.enum(["PCT", "USD", "COUNT", "MS", "PRICE"]),
      warnAt: z.number(),
      breachAt: z.number(),
      breached: z.boolean(),
      changedAt: timestamp,
  ...
  breakers: z.array(
    z.object({
      key: text,
      label: text,
      state: z.enum(["CLOSED", "WARNING", "OPEN", "RECOVERING", "MANUAL_LOCK"]),
      triggerCondition: text,
      currentValue: text,
      threshold: text,
      triggeredAt: timeOrNull,
      recoveryCondition: text,
      manualResetAllowed: z.boolean(),
      lastResetAt: timeOrNull,
    }),
  ),
  reconciliation: z.object({
    state: z.enum(["IDLE", "RUNNING", "OK", "ISSUES", "FAILED"]),
    lastRunAt: timeOrNull,
    brokerOrders: z.number(),
    runtimeOrders: z.number(),
    brokerPositions: z.number(),
    runtimePositions: z.number(),
    missingOrders: z.number(),
    unknownOrders: z.number(),
    positionMismatches: z.number(),
    volumeMismatches: z.number(),
    stateMismatches: z.number(),
    issues: z.array(
      z.object({
        entity: z.enum(["ORDER", "POSITION"]),
        entityId: text,
        runtimeState: text,
        brokerState: text,
        difference: text,
  ...
  events: z.array(
    z.object({
      id: text,
      at: timestamp,
      severity: z.enum(["TRACE", "DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"]),
      source: text,
      component: text,
      message: text,
      correlationId: text.optional(),
      orderId: text.optional(),
      positionId: text.optional(),
...
export function actionExecutedPayloadSchema() {
  ...
  return z.discriminatedUnion("action", [
    z
      .object({ ...base, action: z.literal("BREAK_EVEN"), detail: breakEvenDetail.optional() })
      .passthrough(),
    z
      .object({ ...base, action: z.literal("TRAILING_MOVE"), detail: trailingMoveDetail })
      .passthrough(),
    z.object({ ...base, action: z.literal("PARTIAL_TP"), detail: partialTpDetail }).passthrough(),
    z
      .object({ ...base, action: z.literal("TIME_EXIT"), detail: timeExitDetail.optional() })
      .passthrough(),
  ]);
}
...
const payloadShapes: Array<[RegExp, z.ZodTypeAny]> = [
  ...
  [
    /^order\./,
    z
      .object({ orderId: z.string(), status: z.string().optional(), symbol: z.string().optional() })
      .passthrough(),
  ],
  ...
  [
    ...
    z.object({ key: z.string(), state: z.string().optional() }).passthrough(),
  ],
  [/^reconciliation\./, z.object({ reconciliationRunId: z.string().optional() }).passthrough()],
  [
    /^risk\.limit\./,
    z
      .object({ key: z.string(), value: z.number().optional(), threshold: z.number().optional() })
      .passthrough(),
  ],
  [
    /^system\.event_gap\./,
    z.object({ from: z.number(), to: z.number(), missing: z.number() }).passthrough(),
  ],
];
