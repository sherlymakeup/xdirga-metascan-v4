...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Primary operational cockpit for the local trading runtime.",
      },
    ],
  }),
...
export function CockpitPage() {
  ...
  const canStart =
    runtime.state === "STOPPED" || runtime.state === "ERROR" || runtime.state === "DISCONNECTED";
      ...
      <div className="mx-auto max-w-[1600px] space-y-3 p-3 md:p-4">
        ...
        {connection.state !== "CONNECTED" && (
          <div className="panel p-3 text-xs text-muted-foreground">
            {connection.state === "CONNECTING"
              ...
              : `Dashboard ${connection.state.toLowerCase()}`}
          </div>
        )}
        {!snap.accountAvailable && (
          <div className="panel p-3 text-xs text-muted-foreground">Account unavailable</div>
        )}
              ...
              <div className="flex flex-wrap items-center gap-2 text-[11px] text-muted-foreground">
                ...
                <span>
                  ...
                  <span className="num text-foreground">
                    {runtime.uptimeSec == null ? "—" : fmtDuration(runtime.uptimeSec)}
                  </span>
                </span>
                <span>
                  started{" "}
                  <span className="num text-foreground">
                    {runtime.startedAt ? relativeTime(runtime.startedAt) : "—"}
                  </span>
                </span>
                <span>
                  last sync{" "}
                  <span className="num text-foreground">
                    {snap.broker.lastRequestAt ? relativeTime(snap.broker.lastRequestAt) : "—"}
                  </span>
                </span>
                <span>
                  account observed{" "}
              ...
              <ControlButton
                ...
                onClick={() =>
                  setConfirm({
                    ...
                    title: "Start runtime",
                    description:
                      "The runtime will connect to the broker gateway and begin evaluating strategies.",
                    impact: (
                      <span>
                        Trading mode: {runtime.tradingMode}. Entries will remain disabled until
                        confirmed.
                      </span>
                    ),
              ...
              <ControlButton
                ...
                onClick={() =>
                  setConfirm({
                    ...
                    title: "Stop runtime",
                    description:
                      "Runtime will finalize current operations and transition to STOPPED.",
                    impact: (
                    ...
                    impact: (
                      <div className="space-y-1">
                        ...
                        <div>
                          ...
                          {snap.orders.filter((o) => o.status === "ACKNOWLEDGED").length})
                        </div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
          ...
          <MetricCard
            label="Floating PnL"
            value={
              !accountAvailable || snap.account.floatingPnl == null
                ? "—"
                : fmtMoney(snap.account.floatingPnl)
            }
            tone={
              !accountAvailable || snap.account.floatingPnl == null
                ? "neutral"
                : snap.account.floatingPnl >= 0
                  ? "ok"
                  : "crit"
            }
            hint={
              loading || !accountAvailable || snap.account.openPositions == null
                ? undefined
                : `${snap.account.openPositions} open`
            }
            loading={loading}
          />
          <MetricCard
            ...
            tone={
              !accountAvailable || snap.account.realizedPnlToday == null
                ? "neutral"
                : snap.account.realizedPnlToday >= 0
                  ? "ok"
                  : "crit"
            }
            hint={
              loading || !accountAvailable
                ? undefined
                : `${fmtNum(snap.account.tradesToday, 0)} trades · win ${fmtNum(snap.account.winRate, 1)}%`
            }
            loading={loading}
          />
          <MetricCard
            ...
            tone={
              !accountAvailable || snap.account.dailyDrawdown == null
                ? "neutral"
                : snap.account.dailyDrawdown < -300
                  ? "crit"
                  : snap.account.dailyDrawdown < 0
                    ? "warn"
                    : "ok"
            }
            hint={
              loading || !accountAvailable ? undefined : `max ${fmtPct(snap.account.maxDrawdown)}`
            }
            loading={loading}
          />
          <MetricCard
            ...
            tone={
              !accountAvailable || snap.account.riskUtilization == null
                ? "neutral"
                : snap.account.riskUtilization > 80
                  ? "crit"
                  : snap.account.riskUtilization > 60
                    ? "warn"
                    : "ok"
            }
            hint={
              loading || !accountAvailable
                ? undefined
                : `margin lvl ${fmtNum(snap.account.marginLevel, 0)}%`
            }
            loading={loading}
          />
        </div>
            ...
            {(critical > 0 || openExec > 0) && (
              <div className="rounded-sm border border-status-crit/40 bg-status-crit/10 p-3 text-xs">
                ...
                <ul className="mt-2 space-y-1 text-status-crit">
                  {critical > 0 && (
                    <li>
                      · {critical} unresolved CRITICAL alert{critical > 1 ? "s" : ""}
                    </li>
                  )}
                  {openExec > 0 && (
                    <li>
                      · {openExec} order{openExec > 1 ? "s" : ""} in EXECUTION_UNKNOWN — do not
                      retry
                    </li>
                  )}
                </ul>
      ...
      <div className="flex items-center gap-1 border-b border-panel-border px-3 py-1.5">
        ...
        <div className="num ml-auto text-[11px] text-muted-foreground">
          ...
          <span className="text-foreground">
            {hasData ? fmtMoney(values[values.length - 1]) : "—"}
          </span>
        </div>
      </div>
      <div className="relative h-56 w-full px-2 py-2">
        {hasData ? (
          ...
          <EmptyState
            ...
            description="No authoritative equity history is available."
          />
        )}
      </div>
    </Panel>
        ...
        <>
                  ...
                  <tr key={p.id} className="border-b border-panel-border/60 hover:bg-muted/40">
                    ...
                    <td className="num px-2 py-1.5 text-right text-muted-foreground">
                      {p.stopLoss ? (
                        ...
                        <span className="text-status-crit">—</span>
                      )}{" "}
                      / {p.takeProfit ? fmtPrice(p.takeProfit) : "—"}
                    </td>
                    <td
                      className={`num px-2 py-1.5 text-right ${p.floatingPnl >= 0 ? "text-profit" : "text-loss"}`}
                    >
          ...
          <div className="grid gap-2 p-2 md:hidden">
            {positions.map((p) => (
              <div
                key={p.id}
                className="rounded-sm border border-panel-border bg-panel-elevated p-2.5"
              >
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`num text-sm ${p.floatingPnl >= 0 ? "text-profit" : "text-loss"}`}
                  >
                    {fmtMoney(p.floatingPnl)}
                  </span>
                </div>
                <div className="mt-1 grid grid-cols-3 gap-2 text-[11px] text-muted-foreground">
                  <div>
                    Entry <div className="num text-foreground">{fmtPrice(p.entryPrice)}</div>
                  </div>
                  <div>
                    Now <div className="num text-foreground">{fmtPrice(p.currentPrice)}</div>
                  </div>
                  <div>
                    SL{" "}
                    <div className={`num ${p.stopLoss ? "text-foreground" : "text-status-crit"}`}>
                      {p.stopLoss ? fmtPrice(p.stopLoss) : "—"}
                    </div>
                  </div>
                </div>
      ...
      <div className="grid grid-cols-2 gap-1.5 md:grid-cols-3 lg:grid-cols-5">
        {subsystems.map((s) => {
          ...
          return (
            <div
              key={s.key}
              className="rounded-sm border border-panel-border bg-panel-elevated p-2"
            >
              <div className="flex items-center justify-between">
...
function ActiveStrategyPanel({ snap }: { snap: ReturnType<typeof useSnapshot> }) {
    ...
    return [
      {
        ...
        warnAt: 30,
        breachAt: 20,
        disp: `${(s.confidence * 100).toFixed(0)}%`,
        inverted: true,
      },
      {
  ...
  return (
    <Panel
      ...
      toolbar={
        <StatusBadge
          tone={s.status === "ACTIVE" ? "ok" : s.status === "PAUSED" ? "warn" : "neutral"}
          size="sm"
        >
          {s.status}
        </StatusBadge>
    ...
    >
      <div className="space-y-2.5 text-[11.5px]">
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-muted-foreground">
          <span>
            ID <span className="num text-foreground">{s.id}</span>
          </span>
          <span>
            v<span className="num text-foreground">{s.version}</span>
          </span>
          <span>
        ...
        <div className="grid grid-cols-3 gap-2">
          <div>
            ...
            <StatusBadge
              tone={
                s.currentBias === "LONG" ? "ok" : s.currentBias === "SHORT" ? "crit" : "neutral"
              }
              size="sm"
            >
              {s.currentBias}
          ...
          <div>
            ...
            <div className={`num ${s.pnlToday >= 0 ? "text-profit" : "text-loss"}`}>
              {fmtMoney(s.pnlToday)}
            </div>
          </div>
          <div>
        ...
        <ul className="divide-y divide-panel-border">
          {alerts.map((a) => {
            ...
            return (
              <li
                key={a.id}
                className={`p-2.5 text-[11.5px] ${a.acknowledged ? "opacity-60" : ""}`}
              >
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 flex items-center justify-between text-[10.5px] text-muted-foreground">
                      <span>
                        {a.source} · {relativeTime(a.createdAt)}
                      </span>
                      {isDemo && !a.acknowledged && (
                        <button
                          onClick={() =>
                            getRuntimeAdapter().sendCommand({ kind: "alert.acknowledge", id: a.id })
                          }
      ...
      <ol className="divide-y divide-panel-border">
        {events.map((e) => {
          ...
          return (
            <li
              key={e.id}
              className="grid grid-cols-[auto_auto_1fr] items-baseline gap-2 px-3 py-1.5 text-[11px]"
            >
