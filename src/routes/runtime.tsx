        ...
        <div className="grid gap-3 lg:grid-cols-3">
          ...
          <Panel title="Operations" subtitle="Capability-gated">
            <div className="space-y-1.5">
              <CommandButton kind="runtime.start" label="Start runtime" fullWidth variant="primary" />
              ...
              <div className="my-1 border-t border-panel-border" />
              <CommandButton
                kind={snap.runtime.entriesEnabled ? "runtime.disableEntries" : "runtime.enableEntries"}
              ...
              {snap.subsystems.map((s) => (
                <tr key={s.key} className="border-b border-panel-border/60">
                  ...
                  <td className="px-2 py-1.5">
                    <StatusBadge tone={s.state === "OK" ? "ok" : s.state === "DEGRADED" ? "warn" : s.state === "DOWN" ? "crit" : "neutral"} size="sm">
                      {s.state}
                    </StatusBadge>
                  </td>
                  <td className="num px-2 py-1.5 text-muted-foreground">{relativeTime(s.lastHeartbeatAt)}</td>
                  <td className="num px-2 py-1.5 text-right">{s.latencyMs != null ? `${s.latencyMs}ms` : "—"}</td>
              ...
              <tbody>
                {snap.events.slice(0, 40).map((e) => (
                  <tr key={e.id} className="border-b border-panel-border/60">
                    <td className="num w-24 px-2 py-1 text-muted-foreground">{relativeTime(e.at)}</td>
                    <td className="w-20 px-2 py-1">
                      <StatusBadge
                        tone={e.severity === "CRITICAL" || e.severity === "ERROR" ? "crit" : e.severity === "WARNING" ? "warn" : "info"}
                        size="sm"
                      >
                        {e.severity}
                      </StatusBadge>
...
function ReconciliationPanel({ r }: { r: ReconciliationSummary }) {
  const stateTone: StatusTone =
    r.state === "OK" ? "ok" : r.state === "RUNNING" ? "info" : r.state === "ISSUES" ? "warn" : r.state === "FAILED" ? "crit" : "neutral";
  return (
    <Panel
    ...
    >
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <ReconTile label="Broker orders" value={r.brokerOrders} diff={r.brokerOrders - r.runtimeOrders} />
        <ReconTile label="Runtime positions" value={r.runtimePositions} />
        <ReconTile label="Broker positions" value={r.brokerPositions} diff={r.brokerPositions - r.runtimePositions} />
        ...
        <ReconTile label="Position mismatches" value={r.positionMismatches} warn={r.positionMismatches > 0} />
        <ReconTile label="Volume mismatches" value={r.volumeMismatches} warn={r.volumeMismatches > 0} />
      </div>
            ...
            <tbody>
              {r.issues.map((i, idx) => (
                <tr key={idx} className="border-b border-panel-border/60">
                  ...
                  <td className="num px-2 py-1">{i.entity} {i.entityId}</td>
...
function ReconTile({ label, value, diff, warn }: { label: string; value: number; diff?: number; warn?: boolean }) {
  return (
    <div className="rounded-sm border border-panel-border bg-panel-elevated p-2">
      <div className="text-[10px] uppercase tracking-wider text-muted-foreground">{label}</div>
      <div className={`num mt-0.5 text-lg font-semibold ${warn ? "text-status-warn" : "text-foreground"}`}>{value}</div>
      {diff != null && diff !== 0 && (
...
function KV({ label, value, full }: { label: string; value: React.ReactNode; full?: boolean }) {
  return (
    <div className={`flex items-baseline justify-between gap-2 border-b border-panel-border/50 py-1 text-[11.5px] ${full ? "md:col-span-2" : ""}`}>
