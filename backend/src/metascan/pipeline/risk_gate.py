...
def run_gates(request: InternalEntryRequest, facts: RuntimeFacts, config: RiskConfig, provider: RuntimeFactsProvider | None = None) -> GateResult:
    ...
    if request.stopLoss is not None and entry_price is not None and ((request.side == "BUY" and request.stopLoss >= entry_price) or (request.side == "SELL" and request.stopLoss <= entry_price)): return _result(False, "VALIDATION_FAILED", trace)
